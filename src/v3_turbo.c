...
// Brutal 模式配置
typedef struct {
    uint64_t    target_bps;         // 目标速率 (bps)
    bool        brutal_enabled;     // 是否启用 Brutal
    uint8_t     xor_group_size;     // XOR FEC 组大小
} turbo_config_t;

static turbo_config_t g_config = {
    .target_bps = 100 * 1000 * 1000,    // 默认 100 Mbps
    .brutal_enabled = true,
    .xor_group_size = 4,
};
...
static uint64_t brutal_acquire(brutal_pacer_t *p, size_t bytes) {
    ...
    // 限制最大突发
    uint64_t max_burst = p->target_bps / 8 / 10;  // 100ms 突发
...
static void handle_packet(io_ctx_t *ctx, int len) {
    ...
    if (g_fec_group.count >= g_config.xor_group_size) {
        ...
        size_t parity_len = xor_fec_generate_parity(&g_fec_group, parity);
