...
static void chacha20_xor(uint8_t *out, const uint8_t *in, size_t len, 
    ...
    // 加载 Key (Little Endian，但在 byte array转换时通常直接memcpy)
...
// --- AEAD Encrypt/Decrypt (RFC 7539 Construction) ---
...
static int aead_encrypt(uint8_t *ct, uint8_t tag[16], 
    ...
    // 1. 生成 Poly1305 One-Time Key (Subkey)
    // 使用 ChaCha20 Block 0 (Counter=0)
    uint8_t poly_key[64] = {0}; // 需要生成 64 字节块，但只取前 32
    ...
    // 3. 计算 MAC
    // Input: AAD | pad16(AAD) | Ciphertext | pad16(Ciphertext) | len(AAD) | len(Ciphertext)
    poly1305_state st;
...
typedef struct __attribute__((packed)) {
    ...
    uint8_t nonce[12]; 
    uint8_t enc_block[16]; // Encrypted Metadata
    uint8_t tag[16];       // AEAD Tag
    uint16_t early_len;    // AAD Field
    uint16_t pad;
} v3_header_t;
...
typedef struct {
	...
	uint16_t stream_id; 
	uint16_t flags;
	uint16_t early_len; // duplicated for internal use
} v3_meta_t;
...
static uint32_t derive_magic(time_t window) {
    ...
    uint64_t w = window / 60; // 1 minute window
    memcpy(input + 32, &w, 8);
...
static int decrypt_header(const uint8_t *buf, size_t len, v3_meta_t *out) {
    ...
    // 2. Construct AAD (Authenticated Additional Data)
    // 防止重放攻击通常需要 Session ID 或 Timestamp，这里使用 header 中的公开字段
    // AAD = early_len | pad | magic
    uint8_t aad[6]; 
    memcpy(aad, &hdr->early_len, 2); 
    memcpy(aad+2, &hdr->pad, 2); 
    memcpy(aad+4, &hdr->magic_derived, 4); // Oops, magic is 4 bytes
    // Re-check alignment:
    // early_len(2) + pad(2) + magic(4) = 8 bytes AAD
    uint8_t aad_buf[8];
    memcpy(aad_buf, &hdr->early_len, 2);
    memcpy(aad_buf+2, &hdr->pad, 2);
    memcpy(aad_buf+4, &hdr->magic_derived, 4);

    // 3. Decrypt Metadata Block
    uint8_t plaintext[16]; // v3_meta is slightly different layout in struct, but protocol uses 16 bytes
    // Decrypting 16 bytes of encrypted metadata
    ...
    // 4. Parse Metadata
    // Protocol definition: [Session(8)] [Intent(2)] [Stream(2)] [Flags(2)] [Pad(2)]
...
static void handle_udp_packet(uint8_t *buf, int len, struct sockaddr_in *from) {
    ...
    // 实际转发逻辑需要维护 Session Map 和 NAT，此处仅演示解包
    // ...
}
