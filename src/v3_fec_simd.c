...
// =========================================================
// GF(2^8) 基础 & Nibble Table (真 SIMD 核心)
// =========================================================
static uint8_t gf_exp[512];
static uint8_t gf_log[256];

// Nibble Tables: 8KB total (L1 Cache Friendly)
// low[c][i] = c * i
// high[c][i] = c * (i << 4)
...
// AVX2 真 SIMD 实现 (32 字节/次，Nibble Lookup with pshufb)
...
static void gf_mul_add_region_avx512(uint8_t *dst, const uint8_t *src, uint8_t coef, size_t len) {
    ...
    // 复制 nibble 表到 512-bit（4 个 128-bit lane）
    __m128i tbl_lo_128 = _mm_loadu_si128((__m128i*)gf_mul_low[coef]);
    __m128i tbl_hi_128 = _mm_loadu_si128((__m128i*)gf_mul_high[coef]);
...
#ifdef HAVE_NEON
// ARM NEON 真 SIMD 实现 (16 字节/次，Nibble Lookup with vqtbl1q)
...
static void rs_encode_dispatch(fec_type_t type,
    ...
    for (int p = 0; p < parity_count; p++) {
        uint8_t x = data_count + p + 1; // x 从 data_count+1 开始
        matrix[p][0] = 1;
...
static int rs_decode_common(fec_type_t type,
    ...
    for (int col = 0; col < data_count; col++) {
        ...
        // Scale
        uint8_t scale = gf_exp[255 - gf_log[matrix[col][col]]]; // 1/pivot
        for (int j = 0; j < data_count; j++) {
...
int fec_encode(fec_engine_t *e,
    ...
    *group_id = e->next_group_id++;
    ...
    // Align shard_size to 64 bytes for AVX-512 preference
    ...
    for (int i = 0; i < ds; i++) {
        ...
        out_shards[i][5] = ds;
        out_shards[i][6] = ps;
        out_shards[i][7] = (shard_size >> 4) & 0xFF; // Stored as multiples of 16
